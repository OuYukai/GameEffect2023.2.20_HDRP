...
Shader "ObjectFlame"
{
	Properties
	{
		...
		_F_power("F_power", Range( 0 , 5)) = 2
		_Y_mask("Y_mask", Range( 0 , 5)) = 1
		[HDR]_FlameColor1("Flame Color 1", Color) = (1,0.7189842,0,0)
		[HDR]_FlameColor2("Flame Color 2", Color) = (0.6698113,0.07849351,0,0)
		_Y_maskSub("Y_maskSub", Range( 0 , 5)) = 1
		_Flame_height("Flame_height", Range( 0 , 1)) = 0
		_Flame_Wave("Flame_Wave", 2D) = "white" {}
		_Flame_Noise("Flame_Noise", 2D) = "white" {}
		_U("U", Range( -5 , 5)) = 0
		_V("V", Range( -5 , 5)) = -0.5
		_Flame_on("Flame_on", Range( 0 , 1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _Flame_Wave;
			sampler2D _Flame_Noise;
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				float4 localWorldVar33 = float4( float3(0,1,0) , 0.0 );
				(localWorldVar33).xyz = GetCameraRelativePositionWS((localWorldVar33).xyz);
				float4 transform33 = mul(GetWorldToObjectMatrix(),localWorldVar33);
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = inputMesh.ase_texcoord.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
				float4 temp_output_47_0 = ( tex2Dlod( _Flame_Wave, float4( panner40, 0, 0.0) ) * tex2Dlod( _Flame_Noise, float4( panner40, 0, 0.0) ) );
				float4 lerpResult39 = lerp( float4( 0,0,0,0 ) , transform33 , temp_output_47_0);
				float3 ase_normalWS = TransformObjectToWorldNormal(inputMesh.normalOS);
				float clampResult28 = clamp( ( distance( ase_normalWS.y , _Y_mask ) - _Y_maskSub ) , 0.0 , 1.0 );
				float temp_output_30_0 = ( 1.0 - clampResult28 );
				float4 lerpResult34 = lerp( float4( 0,0,0,0 ) , lerpResult39 , temp_output_30_0);
				...
				#endif
				float3 vertexValue = ( lerpResult34 * _Flame_height ).xyz;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
					...
					)
			{
				...
				float fresnelNode8 = ( _F_bias + _F_scale * pow( 1.0 - fresnelNdotV8, _F_power ) );
				float4 lerpResult18 = lerp( _FlameColor2 , _FlameColor1 , fresnelNode8);
				
				float clampResult28 = clamp( ( distance( ase_normalWS.y , _Y_mask ) - _Y_maskSub ) , 0.0 , 1.0 );
				float temp_output_30_0 = ( 1.0 - clampResult28 );
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = packedInput.ase_texcoord3.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
				float4 temp_output_47_0 = ( tex2D( _Flame_Wave, panner40 ) * tex2D( _Flame_Noise, panner40 ) );
				float4 lerpResult49 = lerp( float4( 0,0,0,0 ) , ( fresnelNode8 * temp_output_30_0 * temp_output_47_0 ) , _Flame_on);
				
				surfaceDescription.Color = lerpResult18.rgb;
				surfaceDescription.Emission = 0;
				surfaceDescription.Alpha = lerpResult49.r;
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput inputMesh  )
			{
				...
				float4 transform33 = mul(GetWorldToObjectMatrix(),localWorldVar33);
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = inputMesh.uv0.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
				float4 temp_output_47_0 = ( tex2Dlod( _Flame_Wave, float4( panner40, 0, 0.0) ) * tex2Dlod( _Flame_Noise, float4( panner40, 0, 0.0) ) );
				float4 lerpResult39 = lerp( float4( 0,0,0,0 ) , transform33 , temp_output_47_0);
				float3 ase_normalWS = TransformObjectToWorldNormal(inputMesh.normalOS);
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				float temp_output_30_0 = ( 1.0 - clampResult28 );
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = packedInput.ase_texcoord4.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
				float4 temp_output_47_0 = ( tex2D( _Flame_Wave, panner40 ) * tex2D( _Flame_Noise, panner40 ) );
				float4 lerpResult49 = lerp( float4( 0,0,0,0 ) , ( fresnelNode8 * temp_output_30_0 * temp_output_47_0 ) , _Flame_on);
		...
		Pass
		{
					...
					)
			{
				...
				float temp_output_30_0 = ( 1.0 - clampResult28 );
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = packedInput.ase_texcoord2.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
				float4 temp_output_47_0 = ( tex2D( _Flame_Wave, panner40 ) * tex2D( _Flame_Noise, panner40 ) );
				float4 lerpResult49 = lerp( float4( 0,0,0,0 ) , ( fresnelNode8 * temp_output_30_0 * temp_output_47_0 ) , _Flame_on);
        ...
        Pass
		{
					...
					)
			{
				...
				float3 ase_viewVectorWS = ( _WorldSpaceCameraPos.xyz - ase_positionWS );
				float3 ase_viewDirWS = normalize( ase_viewVectorWS );
				float fresnelNdotV8 = dot( packedInput.normalWS, ase_viewDirWS );
				float fresnelNode8 = ( _F_bias + _F_scale * pow( 1.0 - fresnelNdotV8, _F_power ) );
				float clampResult28 = clamp( ( distance( packedInput.normalWS.y , _Y_mask ) - _Y_maskSub ) , 0.0 , 1.0 );
				float temp_output_30_0 = ( 1.0 - clampResult28 );
				float2 appendResult46 = (float2(_U , _V));
				float2 uv_Flame_Noise = packedInput.ase_texcoord3.xy * _Flame_Noise_ST.xy + _Flame_Noise_ST.zw;
				float2 panner40 = ( 1.0 * _Time.y * appendResult46 + uv_Flame_Noise);
...
/*ASEBEGIN
Version=19800
Node;AmplifyShaderEditor.RangedFloatNode;43;-3152,672;Inherit;False;Property;_U;U;10;0;Create;True;0;0;0;False;0;False;0;0;-5;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;44;-3152,768;Inherit;False;Property;_V;V;11;0;Create;True;0;0;0;False;0;False;-0.5;0;-5;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.WorldNormalVector;22;-3152,-48;Inherit;True;False;1;0;FLOAT3;0,0,1;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.RangedFloatNode;23;-3216,176;Inherit;False;Property;_Y_mask;Y_mask;3;0;Create;True;0;0;0;False;0;False;1;1;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.DynamicAppendNode;46;-2800,672;Inherit;False;FLOAT2;4;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.TextureCoordinatesNode;41;-2880,384;Inherit;True;0;38;2;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT2;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.DistanceOpNode;24;-2768,0;Inherit;True;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;27;-2864,224;Inherit;False;Property;_Y_maskSub;Y_maskSub;6;0;Create;True;0;0;0;False;0;False;1;1;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.PannerNode;40;-2480,384;Inherit;True;3;0;FLOAT2;0,0;False;2;FLOAT2;1,1;False;1;FLOAT;1;False;1;FLOAT2;0
Node;AmplifyShaderEditor.SimpleSubtractOpNode;25;-2528,0;Inherit;True;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.Vector3Node;32;-2160,688;Inherit;False;Constant;_Vector0;Vector 0;5;0;Create;True;0;0;0;False;0;False;0,1,0;0,0,0;0;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
Node;AmplifyShaderEditor.SamplerNode;37;-2016,240;Inherit;True;Property;_Flame_Wave;Flame_Wave;8;0;Create;True;0;0;0;False;0;False;-1;fb4d1dfc36917734290650fa9e589f59;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SamplerNode;38;-2016,448;Inherit;True;Property;_Flame_Noise;Flame_Noise;9;0;Create;True;0;0;0;False;0;False;-1;578dd30dcb9ad6840a2af3ffd297a8a5;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.ClampOpNode;28;-2304,0;Inherit;True;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;1;FLOAT;0
Node;AmplifyShaderEditor.WorldToObjectTransfNode;33;-1920,688;Inherit;True;1;0;FLOAT4;0,0,0,1;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;47;-1616,416;Inherit;True;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;9;-1472,-272;Inherit;False;Property;_F_bias;F_bias;0;0;Create;True;0;0;0;False;0;False;0.2;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;10;-1472,-208;Inherit;False;Property;_F_scale;F_scale;1;0;Create;True;0;0;0;False;0;False;1.5;0;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;11;-1472,-144;Inherit;False;Property;_F_power;F_power;2;0;Create;True;0;0;0;False;0;False;2;0;0;5;0;1;FLOAT;0
Node;AmplifyShaderEditor.OneMinusNode;30;-1120,0;Inherit;True;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;39;-1344,592;Inherit;True;3;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.FresnelNode;8;-1168,-288;Inherit;True;Standard;WorldNormal;ViewDir;False;False;5;0;FLOAT3;0,0,1;False;4;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;5;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;34;-752,672;Inherit;True;3;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0,0,0,0;False;2;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.RangedFloatNode;35;-800,960;Inherit;False;Property;_Flame_height;Flame_height;7;0;Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;29;-736,-32;Inherit;True;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;48;-736,304;Inherit;False;Property;_Flame_on;Flame_on;12;0;Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;18;-816,-720;Inherit;True;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.NormalVertexDataNode;31;-320,-624;Inherit;True;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;36;-448,672;Inherit;True;2;2;0;FLOAT4;0,0,0,0;False;1;FLOAT;0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.ColorNode;17;-1136,-544;Inherit;False;Property;_FlameColor2;Flame Color 2;5;1;[HDR];Create;True;0;0;0;False;0;False;0.6698113,0.07849351,0,0;0.6698113,0.07849351,0,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.ColorNode;16;-1136,-784;Inherit;False;Property;_FlameColor1;Flame Color 1;4;1;[HDR];Create;True;0;0;0;False;0;False;1,0.7189842,0,0;1,0.7189842,0,0;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.LerpOp;49;-384,-160;Inherit;True;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;0;160,-720;Float;False;True;-1;3;Rendering.HighDefinition.HDUnlitGUI;0;13;ObjectFlame;7f5cb9c3ea6481f469fdd856555439ef;True;Forward Unlit;0;0;Forward Unlit;9;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;3;RenderPipeline=HDRenderPipeline;RenderType=Transparent=RenderType;Queue=Transparent=Queue=0;True;5;True;1;d3d11;0;False;False;False;False;True;3;1;False;;10;False;;0;1;False;;0;False;;False;False;True;1;1;False;;0;True;_DstBlend2;0;1;False;;0;False;;False;False;True;1;1;False;;0;True;_DstBlend2;0;1;False;;0;False;;False;False;False;True;0;True;_CullModeForward;False;False;False;True;True;True;True;True;0;True;_ColorMaskTransparentVel;False;False;False;False;False;True;True;0;True;_StencilRef;255;False;;255;True;_StencilWriteMask;7;False;;3;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;True;0;True;_ZWrite;True;0;True;_ZTestDepthEqualForOpaque;False;True;1;LightMode=ForwardOnly;False;False;0;Hidden/InternalErrorShader;0;0;Standard;30;Surface Type;1;638716628220435560;  Rendering Pass ;0;0;  Rendering Pass;1;0;  Blending Mode;0;0;  Receive Fog;0;638716632444260305;  Distortion;0;0;    Distortion Mode;0;0;    Distortion Only;1;0;  Depth Write;1;0;  Cull Mode;0;0;  Depth Test;4;0;Double-Sided;0;0;Alpha Clipping;0;0;Receive Decals;1;0;Motion Vectors;1;0;  Add Precomputed Velocity;0;0;Shadow Matte;0;0;Cast Shadows;1;0;GPU Instancing;1;0;Tessellation;0;0;  Phong;0;0;  Strength;0.5,False,;0;  Type;0;0;  Tess;16,False,;0;  Min;10,False,;0;  Max;25,False,;0;  Edge Length;16,False,;0;  Max Displacement;25,False,;0;Vertex Position,InvertActionOnDeselection;1;0;LOD CrossFade;0;0;0;8;True;True;True;True;True;True;False;True;False;;False;0
...
WireConnection;0;7;31;0
ASEEND*/
//CHKSM=8AACC93107396DF69AF4DED8CF166904A7E6DCF1